...
public class Parqueadero {
    ...
    public String metodo1() {
        // se crea una variable menor con una hora de llegada muy grande para empezar el ciclo
        Carro menor=new Carro("placa vacía",2400);

        for(int i =0; i<this.puestos.length;i++){
            if(this.puestos[i].estaOcupado()){
                Carro carrito=this.puestos[i].darCarro();
                int horaLlegada=carrito.darHoraLlegada();
                if(horaLlegada<menor.darHoraLlegada()){
                    menor=carrito;
                }
            }

        }
        if(menor.darPlaca()=="placa vacía"){return "no hay carros en el parqueadero";}
       return menor.darPlaca();
    }
    ...
    public String metodo2() {
        for(int i=0; i<this.puestos.length;i++){
            if(!this.puestos[i].estaOcupado()){
                return "Hay parqueaderos disponibles";
            }
        }
        return "El parqueadero esta lleno";
    }
